...
> 本文件只規定「怎麼做」，不含設計理由。設計歷程與理由見交接文件 HANDOFF.md（不在本 repository 內，另外保存）。
...
三條界線：Rules 不存知識本身／Knowledge 不預先建立關係也不含專案決策／每層更新互不強制連動（見第 6 節）。
...
**自包含原則**：可以引用其他 Knowledge（例如「詳見 xxx.md」），但不可以要求先讀懂別份才能理解本份。
...
- 新的組合方式，可能被不同場景重複用到 → 走第 4 節，新增／更新 System
- 新的基本原理，可能被不同場景重複用到 → 走第 3 節，新增／更新 Knowledge
