...
本版進一步確認：v0.1 是完整概念架構，v0.2 是無 API 最小運行邊界，v0.3 將兩者統合為「完整概念模型 + 最小實體化 + 持續蒸餾」。

目前 Repository 文件較多的一個重要原因，是蒸餾程序剛建立，歷史施工資料尚未完成集中蒸餾與歸檔。因此目前文件數量不能直接視為最終介面複雜度。

蒸餾後，目標是將日常工作介面與歷史資料分離：CURRENT 負責現行工作；ARCHIVE 集中保存舊版本、施工紀錄、測試、失敗案例與歷史證據。歷史不刪除，但預設不進入 Context。

本版特別區分三個維度：資料本體、資料狀態、Context 載入層級。三者不能混成同一棵分類樹。

資料本體回答「這是什麼」；資料狀態回答「現在能不能作為工作依據」；Context 載入回答「GPT 現在需要知道多少」。

本版也正式確認 Canonical Source 原則：一個事實應有唯一真相來源，但可以有索引、摘要、交接與導航；禁止的是多個地方同時維護不同真相，而不是禁止同一資訊被引用。

知識原子化不再追求最小碎片，而追求可獨立理解、可獨立引用。系統、圖書館、Intent、Trigger、Blueprint、統御等概念保留，但只有被實際需求或 FIELD 證明的責任才需要提升成正式系統。

無 API 階段暫不建立 Vector Database、Embedding Server、常駐 Memory Server、Agent Runtime、Event Bus、Scheduler、Worker、Multi-Agent Framework、大型 Intent Registry、大型 Trigger Engine 或自動 Orchestrator。

正式化新系統的基本門檻是：人工使用 → Observation → 發現重複瓶頸 → 提出方案 → 壓力測試 → 確認 → 建立 → 回歸。

本版最終原則：大記憶，小 Context；完整歷史，乾淨現在；完整能力，最小實體化。

下一階段優先執行蒸餾，而不是繼續增加架構文件：集中歷史至 ARCHIVE、形成乾淨 CURRENT、檢查入口與 Canonical Source、進行新 GPT 接手測試，之後等待自然 FIELD 再決定下一步。